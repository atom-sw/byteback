...
public class PAG implements PointsToAnalysis {
            ...
            for (int i = 0; i < ret.length; i++) {
                ...
                if (rep != reti || rep == key) {
                    ...
                    if (ret.length <= 75) {
                        ...
                        s.addAll(Arrays.asList(ret).subList(0, i));
    ...
    public LocalVarNode makeLocalVarNode(Object value, Type type, SootMethod method) {
        if (opts.rta()) {
            ...
            type = Scene.v().getObjectType();
            method = null;
        } else if (value instanceof Local val) {
    ...
    public void addCallTarget(Edge e) {
        ...
        if (e.kind() == Kind.GENERIC_FAKE) {
            ...
            if (e.srcUnit() instanceof AssignStmt as) {
            ...
            if (e.srcUnit() instanceof AssignStmt as && (tgt.getReturnType() instanceof RefLikeType)) {
