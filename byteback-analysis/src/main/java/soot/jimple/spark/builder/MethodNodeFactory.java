...
public class MethodNodeFactory extends AbstractShimpleValueSwitch {
        ...
        s.apply(new AbstractStmtSwitch<Node>() {
            ...
            public void caseAssignStmt(AssignStmt as) {
                ...
                if (r instanceof InstanceFieldRef) {
                    ...
                    pag.addDereference((VarNode) getNode());
                } else if (r instanceof StaticFieldRef sfr) {
    ...
    private boolean isReflectionNewInstance(InvokeExpr iexpr) {
        if (iexpr instanceof VirtualInvokeExpr vie) {
          if (vie.getBase().getType() instanceof RefType rt) {
              if (rt.getSootClass().getName().equals("java.lang.Class")) {
                    SootMethodRef ref = vie.getMethodRef();
                    return ref.getName().equals("newInstance") && ref.getParameterTypes().size() == 0;
                }
            }
        }
        return false;
    ...
    public void caseStaticInvokeExpr(StaticInvokeExpr v) {
        ...
        if (v.getArgCount() == 1 && v.getArg(0) instanceof StringConstant classNameConst && ref.name().equals("forName")
