...
public class GeomPointsTo extends PAG {
    ...
    public void parametrize(double spark_run_time) {
        ...
        if (!dump_dir.isEmpty()) {
            ...
            try {
                ...
                logger.debug("[Geom] Analysis log can be found in: " + log_file);
            } catch (FileNotFoundException e) {
                String msg = "[Geom] The dump file: " + log_file + " cannot be created. Abort.";
                logger.debug(msg);
    ...
    public boolean isExceptionPointer(Node v) {
        if (v.getType() instanceof RefType) {
            ClassModel sc = ((RefType) v.getType()).getSootClass();
    ...
    private PointsToSetInternal field_p2set(PointsToSet s, final SparkField f) {
        if (!(s instanceof PointsToSetInternal bases)) {
