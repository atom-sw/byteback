...
public class JasminClass extends AbstractJasminClass {
    ...
    public JasminClass(ClassModel classModel) {
        super(classModel);
    }
    ...
    protected void emitMethodBody(MethodModel method) {
        ...
        if (!(activeBody instanceof StmtBody body)) {
      ...
      body.validate();
        ...
        // Emit code in one pass
        {
            ...
            LOOP:
            for (Iterator<Unit> codeIt = units.iterator(); codeIt.hasNext(); ) {
                ...
                if (enablePeephole) {
                    ...
                    if (!(s instanceof AssignStmt stmt) || !codeIt.hasNext()) {
                        break PEEP;
                    }
                  ...
                  final Value lvalue = stmt.getLeftOp();
                    ...
                    if (!(lvalue instanceof Local) || !nextStmt.getLeftOp().equivTo(rvalue)
                            || !(nextStmt.getRightOp() instanceof AddExpr addexp)) {
                        break PEEP;
                    }
    ...
    void emitAssignStmt(AssignStmt stmt) {
        ...
        if (lvalue instanceof Local l && (rvalue instanceof AddExpr || rvalue instanceof SubExpr)) {
                    ...
                    op.getType().apply(new TypeSwitch<Object>() {
                        ...
                        private void emitIntToTypeCast() {
                            if (ByteType.v().equals(toType)) {
                                ...
                                throw new RuntimeException("invalid toType from int: " + toType + " " + v);
                            }
                        }
