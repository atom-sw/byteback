...
public class SmartMethodInfoFlowAnalysis {
    ...
    protected void handleFlowsToDataStructure(Value base, Value source) {
        ...
        EquivalentValue sourceEqVal;
        if (source instanceof InstanceFieldRef ifr) {
          sourceEqVal = InfoFlowAnalysis.getNodeForFieldRef(sm, ifr.getField(), (Local) ifr.getBase()); // deals with inner
            // fields
        } else {
            sourceEqVal = new CachedEquivalentValue(source);
    ...
    protected List<Value> handleInvokeExpr(InvokeExpr ie, Stmt is) {
        ...
        while (nodeIt.hasNext()) {
            ...
            if (!(nodeEqVal.getValue() instanceof Ref node)) {
          ...
          List<Value> sources = new ArrayList();
            ...
            if (node instanceof ParameterRef) {
                ...
                // source = node;
                // handleInnerField(source);
            } else if (node instanceof ThisRef && ie instanceof InstanceInvokeExpr iie) {
              sources.add(iie.getBase());
                // source = iie.getBase(); // Local
            } else {
            ...
            while (sinksIt.hasNext()) {
                ...
                if (sink instanceof ParameterRef param) {
                  if (param.getIndex() == -1) {
                ...
                } else if (sink instanceof InstanceFieldRef && ie instanceof InstanceInvokeExpr iie) {
                  if (iie.getBase() == thisLocal) {
                        for (Iterator<Value> sourcesIt = sources.iterator(); sourcesIt.hasNext(); ) {
                            Value source = sourcesIt.next();
                            handleFlowsToValue(sink, source);
    ...
    protected void addFlowToCdfg(Stmt stmt) {
        if (stmt instanceof IdentityStmt is) // assigns an IdentityRef to a Local
        {
        ...
        } else if (stmt instanceof ReturnStmt rs) // assigns an Immediate to the "returnRef"
        {
          Value rv = rs.getOp();
        ...
        } else if (stmt instanceof AssignStmt as) // assigns a Value to a Variable
        {
          Value lv = as.getLeftOp();
            ...
            {
                sink = lv;
            } else if (lv instanceof ArrayRef ar) // data flows into the base's data structure
            {
              sink = ar.getBase();
            ...
            {
                sink = lv;
            } else if (lv instanceof InstanceFieldRef ifr) {
            ...
            if (rv instanceof Local) {
                ...
                sources.add(rv);
                interestingFlow = !ignoreThisDataType(rv.getType());
            } else if (rv instanceof ArrayRef ar) // data flows from the base's data structure
            {
              sources.add(ar.getBase());
                ...
                sources.add(rv);
                interestingFlow = !ignoreThisDataType(rv.getType());
            } else if (rv instanceof InstanceFieldRef ifr) {
