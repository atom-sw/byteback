...
public class SynchObliviousMhpAnalysis implements MhpTester, Runnable {
    ...
    public void run() {
      ...
      SparkOptions so = pag.getOpts();
        ...
        if (optionPrintDebug) {
            logger.debug(String.valueOf(mainThread));
        }
        ...
        while (addedNew) {
            ...
            while (it.hasNext()) {
                ...
                if (mayHappenInParallelInternal(someStartMethod, someStartMethod)) {
                    ...
                    it.remove();
                    if (optionPrintDebug) {
                        logger.debug(String.valueOf(someThread));
                    }
                    addedNew = true;
                }
            }
    ...
    public List<ClassModel> getThreadClassList() {
        if (optionThreaded) {
            if (self == null) {
        ...
        List<ClassModel> threadClasses = new ArrayList<ClassModel>();
        int size = threadList.size();
        for (int i = 0; i < size; i++) {
