...
public class InlinerSafetyManager {
        ...
        for (Unit u : target.getActiveBody().getUnits()) {
            ...
            if (st.containsInvokeExpr()) {
                ...
                if (ie1 instanceof SpecialInvokeExpr) {
                    ClassModel containerDeclaringClass = container.getDeclaringClass();
    ...
    private static boolean invokeThrowsAccessErrorIn(ClassModel base, MethodModel inlinee, MethodModel container) {
        ...
        if (inlinee.isProtected()) {
            ...
            return !h.isClassSuperclassOfIncluding(inlineeClass, containerClass)
    ...
    static boolean specialInvokePerformsLookupIn(InvokeExpr ie, ClassModel containerClass) {
