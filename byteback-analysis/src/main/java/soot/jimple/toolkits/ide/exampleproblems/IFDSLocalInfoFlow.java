    ...
    public FlowFunctions<Unit, Local, SootMethod> createFlowFunctionsFactory() {
        return new FlowFunctions<Unit, Local, SootMethod>() {
            ...
            public FlowFunction<Local> getNormalFlowFunction(Unit src, Unit dest) {
                if (src instanceof IdentityStmt is && interproceduralCFG().getMethodOf(src) == Scene.v().getMainMethod()) {
                ...
                if (src instanceof AssignStmt assignStmt) {
                    Value right = assignStmt.getRightOp();
                    if (assignStmt.getLeftOp() instanceof Local leftLocal) {
                        if (right instanceof Local rightLocal) {
