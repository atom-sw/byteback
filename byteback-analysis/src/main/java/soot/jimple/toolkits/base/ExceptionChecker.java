...
import soot.tag.ThrowCreatedByCompilerTag;
...
public class ExceptionChecker extends BodyTransformer {
    ...
    protected boolean isThrowDeclared(Body b, ClassModel throwClass) {
        if (hierarchy == null) {
            hierarchy = new FastHierarchy();
        }
        final ClassModel runtimeExceptionClassModel = Scene.v().getSootClass("java.lang.RuntimeException");
        final ClassModel errorClassModel = Scene.v().getSootClass("java.lang.Error");
        ...
        if (throwClass.equals(runtimeExceptionClassModel) || throwClass.equals(errorClassModel)
                || hierarchy.isSubclass(throwClass, runtimeExceptionClassModel) || hierarchy.isSubclass(throwClass, errorClassModel)) {
            return true;
        }
        ...
        List<ClassModel> exceptions = b.getMethod().getExceptionsUnsafe();
        if (exceptions != null) {
            for (ClassModel nextEx : exceptions) {
