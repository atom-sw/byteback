...
public class TypeResolver {
    ...
    private void initAssignment(DefinitionStmt ds) {
        ...
        if (lhs instanceof Local || lhs instanceof ArrayRef) {
            ...
            } else if (rhs instanceof BinopExpr be) {
    ...
    public class CastInsertionUseVisitor implements IUseVisitor {
         ...
         *
         * @param useType the new type
         * @param stmt    stmt
         * @param old     the old local
         * @param after   True to insert the cast after the statement, false to insert it before
    ...
    protected Type convert(Type t) {
        ...
        } else if (t instanceof ArrayType r) {
     ...
     * @param countOnly whether to count only (no actual changes)
     * @return the visitor
     */
    ...
    public static boolean typesEqual(Type a, Type b) {
        if (a instanceof ArrayType a_ && b instanceof ArrayType b_) {
    ...
    private void split_new() {
        ...
        for (Iterator<Unit> it = units.snapshotIterator(); it.hasNext(); ) {
            ...
            if (stmt instanceof InvokeStmt) {
                ...
                if ((invokeExpr instanceof SpecialInvokeExpr special) && ("<init>".equals(invokeExpr.getMethodRef().getName()))) {
                    for (List<Unit> deflist = defs.getDefsOfAt((Local) special.getBase(), stmt); deflist.size() == 1; ) {
                        Stmt stmt2 = (Stmt) deflist.get(0);

                        if (stmt2 instanceof AssignStmt assign) {
