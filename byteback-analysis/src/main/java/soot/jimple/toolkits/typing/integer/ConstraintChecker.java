...
class ConstraintChecker extends AbstractStmtSwitch {
    ...
    private void handleInvokeExpr(InvokeExpr ie, Stmt invokestmt) {
        ...
        for (int i = 0, e = ie.getArgCount(); i < e; i++) {
            ...
            if (currArg instanceof Local local) {
        ...
        if (ie instanceof DynamicInvokeExpr die) {
            ...
            for (int i = 0, e = die.getBootstrapArgCount(); i < e; i++) {
                ...
                if (currBootstrapArg instanceof Local local) {
                    Type localType = local.getType();
                    if (localType instanceof IntegerType) {
    ...
    public void caseAssignStmt(AssignStmt stmt) {
        ...
        if (l instanceof ArrayRef) {
            ...
            if (baset instanceof ArrayType base) {
                ...
                if (index instanceof Local) {
                    if (!classHierarchy.typeNode(index.getType()).hasAncestor_1(classHierarchy.INT)) {
                        if (fix) {
        ...
        if (r instanceof ArrayRef) {
        ...
        } else if (r instanceof BinopExpr be) {
            // ******** BINOP EXPR ********
            ...
            if (lv instanceof Local) {
                if (lv.getType() instanceof IntegerType) {
                    lop = classHierarchy.typeNode(lv.getType());
                }
            ...
            if (rv instanceof Local) {
                if (rv.getType() instanceof IntegerType) {
                    rop = classHierarchy.typeNode(rv.getType());
                }
            ...
            right = classHierarchy.BOOLEAN;
        } else if (r instanceof InvokeExpr ie) {
        ...
        } else if (r instanceof NewArrayExpr nae) {
            Value size = nae.getSize();

            if (size instanceof Local) {
                if (!classHierarchy.typeNode(size.getType()).hasAncestor_1(classHierarchy.INT)) {
                    if (fix) {
        ...
        } else if (r instanceof NewMultiArrayExpr nmae) {
            ...
            right = classHierarchy.INT;
        } else if (r instanceof NegExpr ne) {
            Value op = ne.getOp();
    ...
    public void caseTableSwitchStmt(TableSwitchStmt stmt) {
        Value key = stmt.getKey();
        if (key instanceof Local keyLocal) {
