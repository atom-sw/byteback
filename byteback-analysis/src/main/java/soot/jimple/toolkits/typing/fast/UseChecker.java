...
public class UseChecker extends AbstractStmtSwitch {
    ...
    public void caseAssignStmt(AssignStmt stmt) {
                ...
                if (rhs instanceof Local) {
                    ...
                    if ((tgType == Scene.v().getObjectType() && rhsType instanceof PrimType) || tgType instanceof WeakObjectType) {
                        ...
                        for (Unit defU : defs.getDefsOfAt(base, stmt)) {
                            if (defU instanceof AssignStmt defUas) {
        ...
        if (rhs instanceof Local) {
            stmt.setRightOp(this.uv.visit(rhs, tlhs, stmt));
        } else if (rhs instanceof ArrayRef aref) {
            Local base = (Local) aref.getBase();
                        ...
                        if (et == null) {
                            ...
                            for (UnitValueBoxPair usePair : uses.getUsesOf(stmt)) {
                                ...
                                if (useStmt.containsInvokeExpr()) {
                                    ...
                                    if (condition instanceof EqExpr expr) {
                                        final Value other;
            ...
            stmt.setRightOp(this.uv.visit(rhs, tlhs, stmt));
        } else if (rhs instanceof InstanceOfExpr ioe) {
            ioe.setOp(this.uv.visit(ioe.getOp(), Scene.v().getObjectType(), stmt));
            stmt.setRightOp(this.uv.visit(rhs, tlhs, stmt));
        } else if (rhs instanceof NewArrayExpr nae) {
            nae.setSize(this.uv.visit(nae.getSize(), IntType.v(), stmt));
            stmt.setRightOp(this.uv.visit(rhs, tlhs, stmt));
        } else if (rhs instanceof NewMultiArrayExpr nmae) {
