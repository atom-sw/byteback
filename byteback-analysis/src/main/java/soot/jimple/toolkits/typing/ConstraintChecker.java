...
class ConstraintChecker extends AbstractStmtSwitch {
    ...
    private void handleInvokeExpr(InvokeExpr ie, Stmt invokestmt) {
        ...
        if (ie instanceof InterfaceInvokeExpr) {
            ...
            for (int i = 0; i < die.getBootstrapArgCount(); i++) {
                if (die.getBootstrapArg(i) instanceof Local local) {
    ...
    public void caseAssignStmt(AssignStmt stmt) {
        ...
        if (r instanceof ArrayRef) {
            ...
            if (!left.hasDescendantOrSelf(baseTy.element())) {
                if (fix) {
                    ...
                    if (lefttype instanceof ArrayType atype) {
        ...
        } else if (r instanceof CastExpr ce) {
            TypeNode castTy = hierarchy.typeNode(ce.getCastType());
            Value op = ce.getOp();
            if (op instanceof Local) {
                TypeNode opTy = hierarchy.typeNode(op.getType());
                try {
        ...
        } else if (r instanceof InstanceOfExpr ioe) {
        ...
        } else if (r instanceof LengthExpr le) {
            ...
            Value op = le.getOp();
            if (op instanceof Local) {
                if (!hierarchy.typeNode(op.getType()).isArray()) {
                    error("Type Error(39)");
                }
            }
            ...
            Value op = ne.getOp();
            if (op instanceof Local) {
                right = hierarchy.typeNode(op.getType());
        ...
        } else if (r instanceof Local loc) {
    ...
    public void caseReturnStmt(ReturnStmt stmt) {
        Value op = stmt.getOp();
        if (op instanceof Local opLocal) {
