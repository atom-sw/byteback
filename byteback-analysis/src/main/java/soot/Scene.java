...
public class Scene {
    ...
    protected Chain<ClassModel> classes = new HashChain<ClassModel>();
    protected Chain<ClassModel> applicationClasses = new HashChain<ClassModel>();
    protected Chain<ClassModel> libraryClasses = new HashChain<ClassModel>();
    protected Chain<ClassModel> phantomClasses = new HashChain<ClassModel>();
    ...
    public void setMainClass(ClassModel m) {
        mainClass = m;
     ...
     * @param c The class to add
     */
    protected void addClassSilent(ClassModel c) {
        synchronized (c) {
            if (c.isInScene()) {
     ...
     * or null otherwise.
     */
    public ClassModel tryLoadClass(String className, int desiredLevel) {
        /*
        ...
        ClassModel toReturn = SootResolver.v().resolveClass(className, desiredLevel);
        setPhantomRefs(false);
        return toReturn;
     ...
     */
    public ClassModel loadClassAndSupport(String className) {
        ClassModel ret = loadClass(className, ClassModel.SIGNATURES);
        if (!ret.isPhantom()) {
            ret = loadClass(className, ClassModel.BODIES);
        }
        return ret;
    }

    public ClassModel loadClass(String className, int desiredLevel) {
        /*
         * if(Options.v().time()) Main.v().resolveTimer.start();
         */
     ...
     *
     * @throws RuntimeException if the Type for this name cannot be found. Use {@link #getRefTypeUnsafe(String)} to check if type is an
     *                          registered RefType.
     */
     ...
     *
     * @param arg             A string description of the type
     * @param phantomNonExist Indicates that a phantom class should be created for the given type string and a Type object should be created
     ...
     *
     * @throws IllegalStateException if the RefType for this class cannot be found. Use {@link #containsType(String)} to check if type is
     *                               registered
     */
     ...
     */
    public ClassModel getSootClassUnsafe(String className) {
     ...
     *
     * @param className       The name of the class to get
     * @param phantomNonExist Indicates that a phantom class should be created if a class with the given name does not exist and phantom refs
     *                        are allowed
     * @return The class if it exists, otherwise null
     */
    public ClassModel getSootClassUnsafe(String className, boolean phantomNonExist) {
        RefType type = nameToClass.get(className);
        if (type != null) {
            synchronized (type) {
                if (type.hasSootClass()) {
                    ClassModel tsc = type.getSootClass();
                    if (tsc != null) {
                        return tsc;
                    }
                }
        ...
        if ((phantomNonExist && allowsPhantomRefs()) || ClassModel.INVOKEDYNAMIC_DUMMY_CLASS_NAME.equals(className)) {
     ...
     */
    public ClassModel getSootClass(String className) {
        ClassModel sc = getSootClassUnsafe(className);
        if (sc != null) {
            return sc;
        }
     ...
     */
    public Chain<ClassModel> getClasses() {
        return classes;
    }
     ...
     * modified.
     */
    public Chain<ClassModel> getApplicationClasses() {
     ...
     */
    public Chain<ClassModel> getLibraryClasses() {
     ...
     * loaded.
     */
    public Chain<ClassModel> getPhantomClasses() {
        return phantomClasses;
    }

    Chain<ClassModel> getContainingChain(ClassModel c) {
    ...
    private void addSootBasicClasses() {
        basicclasses[ClassModel.HIERARCHY] = new HashSet<String>();
        basicclasses[ClassModel.SIGNATURES] = new HashSet<String>();
        basicclasses[ClassModel.BODIES] = new HashSet<String>();

        addBasicClass("java.lang.Object");
        addBasicClass("java.lang.Class", ClassModel.SIGNATURES);

        addBasicClass("java.lang.Void", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Boolean", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Byte", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Character", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Short", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Integer", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Long", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Float", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Double", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Number", ClassModel.SIGNATURES);

        addBasicClass("java.lang.String");
        addBasicClass("java.lang.StringBuffer", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Enum", ClassModel.SIGNATURES);

        addBasicClass("java.lang.Error");
        addBasicClass("java.lang.AssertionError", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Throwable", ClassModel.SIGNATURES);
        addBasicClass("java.lang.Exception", ClassModel.SIGNATURES);
        addBasicClass("java.lang.NoClassDefFoundError", ClassModel.SIGNATURES);
        addBasicClass("java.lang.ReflectiveOperationException", ClassModel.SIGNATURES);
        ...
        addBasicClass("java.lang.NullPointerException", ClassModel.SIGNATURES);
    ...
    private void addSootBasicDotnetClasses() {
        basicclasses[ClassModel.HIERARCHY] = new HashSet<>();
        basicclasses[ClassModel.SIGNATURES] = new HashSet<>();
        basicclasses[ClassModel.BODIES] = new HashSet<>();
    }

    public void addBasicClass(String name) {
        addBasicClass(name, ClassModel.HIERARCHY);
    }
    ...
    public void loadBasicClasses() {
        ...
        for (int i = ClassModel.BODIES; i >= ClassModel.HIERARCHY; i--) {
            for (String name : basicclasses[i]) {
                ClassModel basicClass = tryLoadClass(name, i);
    ...
    protected void addReflectionTraceClasses() {
        ...
        for (String c : classNames) {
            addBasicClass(c, ClassModel.BODIES);
        }
    }

    public Collection<ClassModel> dynamicClasses() {
        List<ClassModel> temp = dynamicClasses;
        if (temp == null) {
        ...
        if (opts.oaat()) {
            ...
            for (String path : opts.process_dir()) {
                for (String cl : SourceLocator.v().getClassesUnder(path)) {
                    ClassModel theClass = loadClassAndSupport(cl);
    ...
    public void loadDynamicClasses() {
        final ArrayList<ClassModel> dynamicClasses = new ArrayList<ClassModel>();
        final Options opts = Options.v();
        ...
        for (Iterator<ClassModel> iterator = dynamicClasses.iterator(); iterator.hasNext(); ) {
            ClassModel c = iterator.next();
            if (!c.isConcrete()) {
                if (opts.verbose()) {
    ...
    protected void prepareClasses() {
        ...
        Chain<ClassModel> processedClasses = new HashChain<ClassModel>();
        while (true) {
            Chain<ClassModel> unprocessedClasses = new HashChain<ClassModel>(getClasses());
            unprocessedClasses.removeAll(processedClasses);
            if (unprocessedClasses.isEmpty()) {
                break;
            }
            processedClasses.addAll(unprocessedClasses);
            for (ClassModel s : unprocessedClasses) {
                if (s.isPhantom()) {
                    continue;
                }
                if (Options.v().app()) {
    ...
    public boolean isExcluded(ClassModel sc) {
    ...
    public boolean isIncluded(ClassModel sc) {
     ...
     */
    public SootMethodRef makeMethodRef(ClassModel declaringClass, String name, List<Type> parameterTypes, Type returnType,
                                       boolean isStatic) {
     ...
     */
    public SootFieldRef makeFieldRef(ClassModel declaringClass, String name, Type type, boolean isStatic) {
        return new AbstractFieldRef(declaringClass, name, type, isStatic);
    }
        ...
        if (mainClass == null) {
            ...
            if (optsMain != null && !optsMain.isEmpty()) {
                ...
                for (String next : Options.v().classes()) {
                    ClassModel c = getSootClass(next);
                ...
                for (ClassModel c : getApplicationClasses()) {
                    boolean declaresMethod = c.declaresMethod("main", mainArgs, VoidType.v());
                    if (declaresMethod) {
                        logger.debug("No main class given. Inferred '" + c.getName() + "' as main class.");
                        setMainClass(c);
     ...
     * finished.
     */
    public ClassModel forceResolve(String className, int level) {
