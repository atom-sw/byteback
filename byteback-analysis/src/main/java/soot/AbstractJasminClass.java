...
public abstract class AbstractJasminClass {
    ...
    private String getVisibilityAnnotationAttr(VisibilityAnnotationTag tag) {
        ...
        if (tag.hasAnnotations()) {
            for (AnnotationTag annot : tag.getAnnotations()) {
                ...
                for (AnnotationElement ae : annot.getElems()) {
    ...
    private String getElemAttr(AnnotationElement elem) {
        StringBuilder result = new StringBuilder(".elem ");
        switch (elem.getKind()) {
            case 'Z': {
                result.append(".bool_kind ");
                result.append('"').append(elem.getName()).append("\" ");
                if (elem instanceof AnnotationIntElement) {
                    result.append(((AnnotationIntElement) elem).getValue());
                } else {
                    result.append(((AnnotationBooleanElement) elem).getValue() ? 1 : 0);
                }
                result.append('\n');
                break;
            }
            ...
            case 'J': {
                ...
                result.append('"').append(elem.getName()).append("\" ");
                result.append(((AnnotationLongElement) elem).getValue());
                result.append('\n');
                break;
            }
            case 'F': {
                ...
                result.append('"').append(elem.getName()).append("\" ");
                result.append(((AnnotationFloatElement) elem).getValue());
                result.append('\n');
                break;
            }
            case 'D': {
                ...
                result.append('"').append(elem.getName()).append("\" ");
                result.append(((AnnotationDoubleElement) elem).getValue());
                result.append('\n');
                break;
            }
            case 's': {
                ...
                result.append('"').append(elem.getName()).append("\" ");
                result.append(StringTools.getQuotedStringOf(((AnnotationStringElement) elem).getValue()));
                result.append('\n');
                break;
            }
            case 'e': {
            ...
            case 'c': {
                ...
                result.append('"').append(elem.getName()).append("\" ");
                result.append(StringTools.getQuotedStringOf(((AnnotationClassElement) elem).getDesc()));
                result.append('\n');
                break;
            }
            case '[': {
                ...
                result.append('"').append(elem.getName()).append("\" ");
                result.append('\n');
                AnnotationArrayElement arrayElem = (AnnotationArrayElement) elem;
            ...
            case '@': {
                ...
                result.append('"').append(elem.getName()).append("\"\n");
                AnnotationTag annot = ((AnnotationSubElement) elem).getValue();
    ...
    public AbstractJasminClass(ClassModel classModel) {
        if (Options.v().time()) {
            Timers.v().buildJasminTimer.start();
        }

        if (Options.v().verbose()) {
            logger.debug("[" + classModel.getName() + "] Constructing baf.JasminClass...");
        }
        ...
        // Emit the header
        {
            int modifiers = classModel.getModifiers();

            if (!Options.v().no_output_source_file_attribute()) {
                SourceFileTag tag = (SourceFileTag) classModel.getTag(SourceFileTag.NAME);
                if (tag != null) {
            ...
            if (Modifier.isInterface(modifiers)) {
                ...
                emit(".interface " + Modifier.toString(modifiers) + " " + slashify(classModel.getName()));
            } else {
                emit(".class " + Modifier.toString(modifiers) + " " + slashify(classModel.getName()));
            }
        ...
        for (Tag tag : classModel.getTags()) {
            if (tag instanceof Attribute) {
                emit(".class_attribute " + tag.getName() + " \"" + String.valueOf(Base64.encode(tag.getValue()))
                        + "\"");
                /*
                 * else { emit(""); }
                 */
        ...
        if (classModel.hasTag(SyntheticTag.NAME) || Modifier.isSynthetic(classModel.getModifiers())) {
            emit(".synthetic\n");
        }
        // emit inner class attributes
        if (!Options.v().no_output_inner_classes_attribute()) {
            InnerClassAttribute ica = (InnerClassAttribute) classModel.getTag(InnerClassAttribute.NAME);
            if (ica != null) {
                List<InnerClassTag> specs = ica.getSpecs();
                if (!specs.isEmpty()) {
                    emit(".inner_class_attr ");
                    for (InnerClassTag ict : specs) {
                      String str = ".inner_class_spec_attr " + '"' + ict.getInnerClass() + "\" " +
                              '"' + ict.getOuterClass() + "\" " +
                              '"' + ict.getShortName() + "\" " +
        ...
        {
            EnclosingMethodTag eMethTag = (EnclosingMethodTag) classModel.getTag(EnclosingMethodTag.NAME);
            if (eMethTag != null) {
              String encMeth = ".enclosing_method_attr " + '"' + eMethTag.getEnclosingClass() + "\" " +
                      '"' + eMethTag.getEnclosingMethod() + "\" " +
                      '"' + eMethTag.getEnclosingMethodSig() + "\"\n";
                emit(encMeth);
            }
        }
        // emit deprecated attributes
        if (classModel.hasTag(DeprecatedTag.NAME)) {
            emit(".deprecated\n");
        }
        {
            SignatureTag sigTag = (SignatureTag) classModel.getTag(SignatureTag.NAME);
            if (sigTag != null) {
                emit(".signature_attr " + "\"" + sigTag.getSignature() + "\"\n");
            }
        }

        for (Tag t : classModel.getTags()) {
        ...
        // Emit the fields
        {
            for (SootField field : classModel.getFields()) {
                ...
                for (Tag tag : field.getTags()) {
                    if (tag instanceof Attribute) {
                        emit(".field_attribute " + tag.getName() + " \"" + String.valueOf(Base64.encode(tag.getValue()))
                                + "\"");
                    }
                }
            }

            if (classModel.getFieldCount() != 0) {
                emit("");
            }
        }
    ...
    protected void emitMethod(SootMethod method) {
        ...
        for (ClassModel exceptClass : method.getExceptions()) {
