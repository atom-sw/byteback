...
public class ThrowableSet {
    ...
    private final ClassModel JAVA_LANG_OBJECT_CLASS = Scene.v().getObjectType().getSootClass();
     ...
     *
     * @param include The set of {@link RefType} and {@link AnySubType} objects representing the types to be included in the set.
     * @param exclude The set of {@link AnySubType} objects representing the types to be excluded from the set.
     */
     ...
     * @throws {@link ThrowableSet.AlreadyHasExclusionsException} if this <code>ThrowableSet</code> is the result of a
    ...
    public ThrowableSet add(RefType e) throws ThrowableSet.AlreadyHasExclusionsException {
        ...
        for (AnySubType excludedType : exceptionsExcluded) {
            ...
            if ((eHasNoHierarchy && exclusionBase.equals(e)) || (!eHasNoHierarchy && hierarchy.canStoreType(e, exclusionBase))) {
                throw new AlreadyHasExclusionsException("ThrowableSet.add(RefType): adding" + e + " to the set [ "
                        + this + "] where " + exclusionBase.toString() + " is excluded.");
            }
        }
    ...
    private boolean hasNoHierarchy(RefType type) {
        final ClassModel classModel = type.getSootClass();
        return !(classModel.hasSuperclass() || JAVA_LANG_OBJECT_CLASS == classModel);
    }
     ...
     * @throws ThrowableSet.AlreadyHasExclusionsException if this <code>ThrowableSet</code> is the result of a {@link #whichCatchableAs(RefType)} operation and, thus,
    ...
    public ThrowableSet add(AnySubType e) throws ThrowableSet.AlreadyHasExclusionsException {
        ...
        final ClassModel objectClass = Scene.v().getObjectType().getSootClass();
        ...
        for (AnySubType excludedType : exceptionsExcluded) {
            ...
            if (isExcluded) {
                ...
                throw new AlreadyHasExclusionsException("ThrowableSet.add(" + e + ") to the set [ " + this
                        + "] where " + exclusionBase + " is excluded.");
            }
        }
     ...
     * @throws ThrowableSet.AlreadyHasExclusionsException if this <code>ThrowableSet</code> or <code>s</code> is the result of a {@link #whichCatchableAs(RefType)}
    ...
    public ThrowableSet add(ThrowableSet s) throws ThrowableSet.AlreadyHasExclusionsException {
        ...
        if ((exceptionsExcluded.size() > 0) || (s.exceptionsExcluded.size() > 0)) {
            throw new AlreadyHasExclusionsException(
                    "ThrowableSet.Add(ThrowableSet): attempt to add to [" + this + "] after removals recorded.");
        }
     ...
     *
     * @param addedExceptions a set of {@link RefLikeType} and {@link AnySubType} objects to be added to the types included in this
     *                        <code>ThrowableSet</code>.
     ...
     *
     * @param removedExceptions a set of {@link RefLikeType} and {@link AnySubType} objects to be added to the types included in this
     *                          <code>ThrowableSet</code>.
     * @return a set containing all the <code>addedExceptions</code> as well as the exceptions in this set.
     */
     ...
     * @param set The <code>Set</code> to which to add the types, or <code>null</code> if no <code>Set</code> has yet been
     *            allocated.
    ...
    public static class Manager {
         ...
         * </p>
         *
         * @param include A set of {@link RefLikeType} objects representing exception types included in the result; may be
         *                <code>null</code> if there are no included types.
         * @param exclude A set of {@link AnySubType} objects representing exception types excluded from the result; may be
        ...
        public String reportInstrumentation() {
          ...
          String buf = "registeredSets: " + setCount + "\naddsOfRefType: " +
                  addsOfRefType + "\naddsOfAnySubType: " + addsOfAnySubType + "\naddsOfSet: " +
                  addsOfSet + "\naddsInclusionFromMap: " + addsInclusionFromMap +
                  "\naddsInclusionFromMemo: " + addsInclusionFromMemo + "\naddsInclusionFromSearch: " +
                  addsInclusionFromSearch + "\naddsInclusionInterrupted: " + addsInclusionInterrupted +
                  "\naddsExclusionWithoutSearch: " + addsExclusionWithoutSearch + "\naddsExclusionWithSearch: " +
                  addsExclusionWithSearch + "\nremovesOfAnySubType: " + removesOfAnySubType +
                  "\nremovesFromMap: " + removesFromMap + "\nremovesFromMemo: " + removesFromMemo +
                  "\nremovesFromSearch: " + removesFromSearch + "\nregistrationCalls: " +
                  registrationCalls + "\ncatchableAsQueries: " + catchableAsQueries +
                  "\ncatchableAsFromMap: " + catchableAsFromMap + "\ncatchableAsFromSearch: " +
                  catchableAsFromSearch + '\n';
            return buf;
        }
    ...
    public static class Pair {
         ...
         *
         * @param caught   The set of exceptions to be returned when {@link #getCaught()} is called on the constructed
         *                 <code>ThrowableSet.Pair</code>.
         * @param uncaught The set of exceptions to be returned when {@link #getUncaught()} is called on the constructed
         *                 <code>ThrowableSet.Pair</code>.
         */
