...
public abstract class BlockGraph implements DirectedBodyGraph<Block> {
     ...
     * @param unitGraph Provides information about the predecessors and successors of each <code>Unit</code> in the <code>Body</code>,
            ...
            if (succUnits.isEmpty()) {
                ...
                if (!tailList.contains(block)) {
                    ...
                    if (block.getPreds().isEmpty() && block.getHead() == block.getTail() && block.getHead() instanceof NopStmt) {
                        ...
                        throw new RuntimeException("Block with no successors is not a tail!: " + block);
     ...
     * @param blockList   The list of blocks for this method. <code>addBlock()</code> will add the newly created block to this list.
     * @param unitToBlock A map from units to blocks. <code>addBlock()</code> will add mappings from <code>head</code> and
