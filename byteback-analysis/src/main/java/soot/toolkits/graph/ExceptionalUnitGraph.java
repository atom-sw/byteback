...
public class ExceptionalUnitGraph extends UnitGraph implements ExceptionalGraph<Unit> {
     ...
     * @param throwAnalysis          the source of information about the exceptions which each {@link Unit} may throw.
     * @param omitExceptingUnitEdges indicates whether the CFG should omit edges to a handler from trapped <code>Unit</code>s which may implicitly
     ...
     *
     * @param body                  the <code>Body</code> from which to build a graph.
     * @param ignoredBogusParameter a meaningless placeholder, which exists solely to distinguish this constructor from the public
     ...
     *
     * @param throwAnalysis          the source of information about the exceptions which each {@link Unit} may throw.
     * @param omitExceptingUnitEdges indicates whether the CFG should omit edges to a handler from trapped <code>Unit</code>s which may throw an
     ...
     * </p>
     *
     * @param throwAnalysis The source of information about which exceptions each <code>Unit</code> may throw.
            ...
            for (Iterator<Unit> unitIt = units.iterator(trap.getBeginUnit(), units.getPredOf(trap.getEndUnit())); unitIt
                ...
                if (!catchableAs.getCaught().equals(ThrowableSet.Manager.v().EMPTY)) {
                    ...
                    assert thrownSet.equals(catchableAs.getUncaught()) : "ExceptionalUnitGraph.buildExceptionDests(): "
                            ...
                            + System.getProperty("line.separator") + " thrownSet == " + thrownSet;
                }
            }
        }
     ...
     *
     * @param map    A <code>Map</code> from <code>Unit</code>s to <code>Collection</code>s of <code>ExceptionDest</code>s.
     *               <code>null</code> if no exceptions have been recorded yet.
     * @param u      The <code>Unit</code> throwing the exceptions.
     * @param t      The <code>Trap</code> which catches the exceptions, or <code>null</code> if the exceptions escape the method.
     * @param caught The set of exception types thrown by <code>u</code> which are caught by <code>t</code>.
     ...
     *
     * @param throwAnalysis          the source of information about the exceptions which each {@link Unit} may throw.
     * @param unitToExceptionDests   A <code>Map</code> from {@link Unit}s to {@link Collection}s of {@link ExceptionalUnitGraph.ExceptionDest
     *                               ExceptionDest}s which represent the handlers that might catch exceptions thrown by the <code>Unit</code>. This
     *                               is an ``in parameter''.
     * @param unitToSuccs            A <code>Map</code> from <code>Unit</code>s to {@link List}s of <code>Unit</code>s. This is an ``out
     ...
     *                               successors.
     * @param unitToPreds            A <code>Map</code> from <code>Unit</code>s to <code>List</code>s of <code>Unit</code>s. This is an ``out
     ...
     * @param omitExceptingUnitEdges Indicates whether to omit exceptional edges from excepting units which lack side effects
    ...
    protected Set<Unit> buildExceptionalEdges(ThrowAnalysis throwAnalysis,
        ...
        class CFGEdge {
            ...
            public boolean equals(Object rhs) {
                ...
                if (!(rhs instanceof CFGEdge rhsEdge)) {
                    return false;
                }
     ...
     *
     * @param u The unit for which to provide exception information. ( <code>u</code> must be a <code>Unit</code>, though the
    ...
    public static class ExceptionDest implements ExceptionalGraph.ExceptionDest<Unit> {
        ...
        public boolean equals(Object obj) {
            ...
            if (trap == null) {
                return other.trap == null;
            } else return trap.equals(other.trap);
        }
