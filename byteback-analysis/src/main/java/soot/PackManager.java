...
import soot.tag.InnerClassTagAggregator;
import soot.tag.LineNumberTagAggregator;
...
public class PackManager {
    ...
    private void runPacksForOneClassAtATime() {
        ...
        for (String path : Options.v().process_dir()) {
            ...
            for (String cl : srcLoc.getClassesUnder(path)) {
                ClassModel clazz = scene.forceResolve(cl, ClassModel.SIGNATURES);
            ...
            for (String cl : srcLoc.getClassesUnder(path)) {
                ...
                try {
                    ...
                    clazz.setResolvingLevel(ClassModel.BODIES);
                ...
                for (ClassModel sc : scene.getApplicationClasses()) {
                    if (validate) {
                        sc.validate();
                    }
                    if (!sc.isPhantom) {
    ...
    private void runPacksNormally() {
        ...
        final boolean validate = Options.v().validate();
        for (ClassModel sc : Scene.v().getApplicationClasses()) {
            if (validate) {
                sc.validate();
            }
            if (!sc.isPhantom) {
    ...
    private void runBodyPacks(final Iterator<ClassModel> classes) {
        ...
        while (classes.hasNext()) {
            final ClassModel c = classes.next();
    ...
    protected void writeOutput(Iterator<ClassModel> classes) {
    ...
    private void releaseBodies(Iterator<ClassModel> classes) {
        while (classes.hasNext()) {
            releaseBodies(classes.next());
        }
    }

    private Iterator<ClassModel> reachableClasses() {
        return Scene.v().getApplicationClasses().snapshotIterator();
    }

    @SuppressWarnings("fallthrough")
    private void runBodyPacks(ClassModel c) {
    ...
    protected void writeClass(ClassModel c) {
        final int format = Options.v().output_format();
        switch (format) {
            case Options.output_format_none:
                return;
     ...
     *
     * @param c The class for which to create a Jasmin-based backend
     * @return The Jasmin-based backend for writing the given class into bytecode
     */
    private AbstractJasminClass createJasminBackend(ClassModel c) {
     ...
     *
     * @param c The class for which to create the ASM backend
     * @return The ASM backend for writing the class into bytecode
     */
    protected BafASMBackend createASMBackend(ClassModel c) {
    ...
    private void retrieveAllBodies() {
        ...
        for (Iterator<ClassModel> clIt = reachableClasses(); clIt.hasNext(); ) {
            ClassModel cl = clIt.next();
