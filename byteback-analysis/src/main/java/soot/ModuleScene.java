...
public class ModuleScene extends Scene {
     ...
     * or null otherwise.
     */
    public ClassModel tryLoadClass(String className, int desiredLevel, Optional<String> moduleName) {
        setPhantomRefs(true);
        ...
        ClassModel toReturn = SootModuleResolver.v().resolveClass(className, desiredLevel, moduleName);
        setPhantomRefs(false);
        return toReturn;
    }
    ...
    public ClassModel loadClassAndSupport(String className, Optional<String> moduleName) {
        ClassModel ret = loadClass(className, ClassModel.SIGNATURES, moduleName);
        if (!ret.isPhantom()) {
            ret = loadClass(className, ClassModel.BODIES, moduleName);
        }
        return ret;
    }
    ...
    public ClassModel loadClass(String className, int desiredLevel, Optional<String> moduleName) {
        /*
     ...
     *
     * @throws RuntimeException if the Type for this name cannot be found. Use {@link #getRefTypeUnsafe(String, Optional)} to check if type is
    ...
    public ClassModel getSootClassUnsafe(String className, Optional<String> moduleName) {
        RefType type = null;
        Map<String, RefType> map = nameToClass.get(className);
        if (map != null && !map.isEmpty()) {
            if (moduleName.isPresent()) {
        ...
        if (allowsPhantomRefs() || ClassModel.INVOKEDYNAMIC_DUMMY_CLASS_NAME.equals(className)) {
    ...
    public ClassModel getSootClass(String className, Optional<String> moduleName) {
        ClassModel sc = getSootClassUnsafe(className, moduleName);
        if (sc != null) {
            return sc;
        }
    ...
    public void loadBasicClasses() {
        ...
        for (int i = ClassModel.BODIES; i >= ClassModel.HIERARCHY; i--) {
            for (String name : basicclasses[i]) {
                ...
                ClassModel classModel = tryLoadClass(wrapper.getClassName(), i, wrapper.getModuleNameOptional());
                if (classModel != null && !classModel.isPhantom()) {
                    loadedClasses++;
                }
            }
        }
            ...
            for (String path : opts.process_dir()) {
                for (Map.Entry<String, List<String>> entry : ModulePathSourceLocator.v().getClassUnderModulePath(path).entrySet()) {
                    for (String cl : entry.getValue()) {
                        ClassModel theClass = loadClassAndSupport(cl, Optional.ofNullable(entry.getKey()));
    ...
    public void loadDynamicClasses() {
        final ArrayList<ClassModel> dynamicClasses = new ArrayList<>();
        final Options opts = Options.v();
    ...
    protected void prepareClasses() {
        ...
        Chain<ClassModel> processedClasses = new HashChain<>();
        while (true) {
            Chain<ClassModel> unprocessedClasses = new HashChain<>(getClasses());
        ...
        if (mainClass == null) {
            ...
            if (optsMain != null && !optsMain.isEmpty()) {
                ...
                for (String s : Options.v().classes()) {
                    ClassModel c = getSootClass(s, null);
    ...
    public ClassModel forceResolve(String className, int level, Optional<String> moduleName) {
    ...
    @Override
    public ClassModel makeSootClass(String className) {
        ModuleUtil.ModuleClassNameWrapper wrapper = ModuleUtil.v().makeWrapper(className);
        return makeSootClass(wrapper.getClassName(), wrapper.getModuleName());
    }

    public ClassModel makeSootClass(String name, String moduleName) {
        return new ClassModel(name, moduleName);
    }
