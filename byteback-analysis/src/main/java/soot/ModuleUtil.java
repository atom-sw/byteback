...
public class ModuleUtil {
    ...
    public final String declaringModule(String className, String toModuleName) {
        if (ModuleInfoModel.MODULE_INFO.equalsIgnoreCase(className)) {
            return toModuleName;
        }

        final ModuleScene modSc = ModuleScene.v();
        final ModuleInfoModel modInfo;
        if (modSc.containsClass(ModuleInfoModel.MODULE_INFO, Optional.ofNullable(toModuleName))) {
            ClassModel temp = modSc.getSootClass(ModuleInfoModel.MODULE_INFO, Optional.ofNullable(toModuleName));
            if (temp.resolvingLevel() < ClassModel.BODIES) {
                modInfo = (ModuleInfoModel) SootModuleResolver.v().resolveClass(ModuleInfoModel.MODULE_INFO, ClassModel.BODIES,
                        Optional.ofNullable(toModuleName));
            } else {
                modInfo = (ModuleInfoModel) temp;
            }
        } else {
            modInfo = (ModuleInfoModel) SootModuleResolver.v().resolveClass(ModuleInfoModel.MODULE_INFO, ClassModel.BODIES,
                    Optional.ofNullable(toModuleName));
        ...
        for (ModuleInfoModel modInf : modInfo.retrieveRequiredModules().keySet()) {
     ...
     */
    private String checkTransitiveChain(ModuleInfoModel modInfo, String packageName, String toModuleName,
                                        Set<String> hasCheckedModule) {
        for (Map.Entry<ModuleInfoModel, Integer> entry : modInfo.retrieveRequiredModules().entrySet()) {
            if ((entry.getValue() & Modifier.REQUIRES_TRANSITIVE) != 0) { // check if module is exported via "requires public"
                final ModuleInfoModel key = entry.getKey();
    ...
    public static final class ModuleClassNameWrapper {
        ...
        private ModuleClassNameWrapper(String className) {
            if (ClassModel.INVOKEDYNAMIC_DUMMY_CLASS_NAME.equals(className)) {
