...
public class PhaseDumper {
    ...
    private static File makeDirectoryIfMissing(Body b) throws IOException {
      String buf = SourceLocator.v().getOutputDir() + File.separatorChar +
              b.getMethod().getDeclaringClass().getName() +
              File.separatorChar +
              b.getMethod().getSubSignature().replace('<', '[').replace('>', ']');
    ...
    private static void deleteOldGraphFiles(final Body b, final String phaseName) {
        try {
            ...
            logger.debug("PhaseDumper.dumpBody() caught: " + e);
     ...
     * @throws IllegalArgumentException if <code>phaseName</code> does not match the <code>PhaseDumper</code>'s record of the current phase.
     */
     ...
     * @param b the {@link Body} represented by <code>g</code>.
     */
