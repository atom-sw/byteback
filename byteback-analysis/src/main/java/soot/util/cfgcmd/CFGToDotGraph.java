...
public class CFGToDotGraph {
     ...
     *
     * @param onePage indicates whether to produce the graph as a single, arbitrarily-sized page (if <code>onePage</code> is
     ...
     * edges.
     *
     * @param graph a <code>DirectedGraph</code> representing a CFG (probably an instance of {@link UnitGraph}, {@link BlockGraph},
     *              or one of their subclasses).
     * @param body  the <code>Body</code> represented by <code>graph</code> (used to format the text within nodes). If no body is
     ...
     *
     * @param body The <code>Body</code> that the graph will represent (used in the graph's title). If no <code>Body</code> is
     *             available, pass <code>null</code>.
        ...
        for (N node : namer.keySet()) {
            ...
            if (printer == null) {
                ...
                for (Unit unit : block) {
                    ...
                    buffer.append(printer).append("\\l");
                }
     ...
     * @param namer   maps from {@link Object} to the strings used to identify corresponding {@link DotGraphNode}s.
