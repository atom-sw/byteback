 ...
 * @author Andreas Dann
 */
public class ModuleInfoModel extends ClassModel {
    ...
    private final Map<ModuleInfoModel, Integer> requiredModules = new HashMap<>();
    ...
    public ModuleInfoModel(String name, int modifiers, String moduleName) {
        super(name, modifiers, moduleName);
    }

    public ModuleInfoModel(String name, String moduleName) {
        this(name, moduleName, false);
    }

    public ModuleInfoModel(String name, String moduleName, boolean isAutomatic) {
    ...
    public Map<ModuleInfoModel, Integer> getRequiredModules() {
        return requiredModules;
    }

    public Map<ModuleInfoModel, Integer> retrieveRequiredModules() {
        Map<ModuleInfoModel, Integer> moduleInfos = requiredModules;

        // move into subclass
        if (this.isAutomaticModule) {
            // we can read all modules
            for (ClassModel classModel : Scene.v().getClasses()) {
                if (classModel instanceof ModuleInfoModel && classModel.moduleName != this.moduleName) {
                    moduleInfos.put((ModuleInfoModel) classModel, Modifier.REQUIRES_STATIC);
                }
            }
        }

        for (ModuleInfoModel moduleInfo : moduleInfos.keySet()) {
            SootModuleResolver.v().resolveClass(ModuleInfoModel.MODULE_INFO, ClassModel.BODIES, Optional.ofNullable(moduleInfo.moduleName));
        }
        return moduleInfos;
    }
    ...
    public boolean opensPackage(String packaze, String toModule) {
        if (ModuleInfoModel.MODULE_INFO.equalsIgnoreCase(packaze)) {
            return true;
        }
    ...
    public Set<ModuleInfoModel> getRequiredPublicModules() {
        Set<ModuleInfoModel> requiredPublic = new HashSet<>();
        // check if exported packages is "requires public"
        for (Map.Entry<ModuleInfoModel, Integer> entry : this.requiredModules.entrySet()) {
