...
/**
 * Provides utility methods to retrieve an input stream for a class name, given a classfile, or jimple or baf output
 * files.
 */
public class SourceLocator {
  ...
  protected void setupClassProviders() {
    ...
    /**
     * Returns the output directory given by {@link Options} or a default if not set. Also ensures that all directories
     * in the path exist.
     *
    ...
    /**
     * If {@link Options#v()#output_jar()} is set, returns the name of the jar file to which the output will be written.
     * The name of the jar file can be given with the -output-dir option or a default will be used. Also ensures that
     * all directories in the path exist.
     *
    ...
    /**
     * Resets the cached class path, class providers, and source path to null. This method allows for subsequent calls
     * to {@link soot.Scene#loadNecessaryClasses()} to recompute and load the classes using updated configurations if
     * provided.
     */
    ...
    static class SharedZipFileCacheWrapper {
        ...
        /**
         * Return the opened {@link ZipFile} wrapped in a {@link SharedCloseable} that is already retained. When the
         * user is done with the {@link ZipFile}, simply call the {@link SharedCloseable#release()} method rather than
         * directly closing the {@link ZipFile} in case there are any other references to it.
         *
         * @param archivePath
         * @return
