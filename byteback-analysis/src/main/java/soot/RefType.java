...
public class RefType extends RefLikeType implements Comparable<RefType> {
    ...
    protected volatile ClassModel classModel;
     ...
     *
     * @param className The name of the class used to parameterize the created RefType.
    ...
    public ClassModel getSootClass() {
        if (classModel == null) {
            ...
            classModel = SootResolver.v().makeClassRef(className);
        }
        return classModel;
    }
     ...
     *
     * @param classModel The SootClass corresponding to this RefType.
     */
    public void setSootClass(ClassModel classModel) {
     ...
     *
     * @param t an object to test for equality. @ return true if t is a RefType parameterized by the same name as this.
     */
    @Override
    ...
    public Type merge(Type other, Scene cm) {
...

        {
            ...
            ArrayDeque<ClassModel> thisHierarchy = new ArrayDeque<>();
            ArrayDeque<ClassModel> otherHierarchy = new ArrayDeque<>();

            // Build thisHierarchy
            // This should never be null, so we could also use "while
            // (true)"; but better be safe than sorry.
            for (ClassModel sc = cm.getSootClass(this.className); sc != null; ) {
            ...
            // This should never be null, so we could also use "while
            // (true)"; but better be safe than sorry.
            for (ClassModel sc = cm.getSootClass(((RefType) other).className); sc != null; ) {
