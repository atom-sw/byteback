...
import soot.tag.ArtificialEntityTag;
...
public class LambdaMetaFactory {
     ...
     * @param tag
     * @param name
     * @param invokedType    types of captured arguments, the last element is always the type of the FunctionalInterface
     * @param enclosingClass
     * @return
     */
    public SootMethodRef makeLambdaHelper(List<? extends Value> bootstrapArgs, int tag, String name, Type[] invokedType,
                                          ClassModel enclosingClass) {
        final int argsSize = bootstrapArgs.size();
        if (argsSize < 3 || !(bootstrapArgs.get(0) instanceof MethodType samMethodType) || !(bootstrapArgs.get(1) instanceof MethodHandle implMethod)
                || !(bootstrapArgs.get(2) instanceof MethodType instantiatedMethodType)
      ...
      int flags = 0;
        ...
        ClassModel functionalInterfaceToImplement = ((RefType) invokedType[invokedType.length - 1]).getSootClass();
     ...
     * @param tclass
     */
    protected void addClassAndInvalidateHierarchy(ClassModel tclass) {
    ...
    private synchronized void resolveHandle(MethodHandle implMethod) {
        ...
        scene.forceResolve(methodRef.getDeclaringClass().getName(), ClassModel.HIERARCHY);

        Stream.concat(Stream.of(methodRef.getReturnType()), methodRef.getParameterTypes().stream())
                .filter(t -> t instanceof RefType)
                .forEach(t -> scene.forceResolve(((RefType) t).getSootClass().getName(), ClassModel.HIERARCHY));
    }
    ...
    private static class Wrapper {
        ...
        public Wrapper() {
            ...
            for (PrimType primTy : primTypes) {
                ...
                ClassModel wrapCls = wrapTy.getSootClass();
                ...
                primitiveValueTmp.put(wrapTy,
                        sc.makeMethodRef(wrapCls, primTy + "Value", Collections.emptyList(), primTy, false).resolve());
            }
    ...
    private class ThunkMethodSource implements MethodSource {
        ...
        public Body getBody(SootMethod m, String phaseName) {
            ...
            ClassModel tclass = m.getDeclaringClass();
         ...
         * @param jb
         */
        private void getInitBody(ClassModel tclass, JimpleBody jb) {
        ...
        private void getBootstrapBody(ClassModel tclass, JimpleBody jb) {
            final Jimple jimp = Jimple.v();
            PatchingChain<Unit> us = jb.getUnits();
            LocalGenerator lc = Scene.v().createLocalGenerator(jb);
         ...
         * @param jb
         */
        private void getInvokeBody(ClassModel tclass, JimpleBody jb) {
            final Jimple jimp = Jimple.v();
            PatchingChain<Unit> us = jb.getUnits();
            LocalGenerator lc = Scene.v().createLocalGenerator(jb);
            ...
            boolean needsReceiver = MethodHandle.Kind.REF_INVOKE_INTERFACE.getValue() == kind
        ...
        private Value _invokeImplMethod(JimpleBody jb, PatchingChain<Unit> us, LocalGenerator lc, List<Local> args) {
            ...
            switch (MethodHandle.Kind.getKind(implMethod.getKind())) {
                ...
                case REF_INVOKE_SPECIAL:
                    final ClassModel currentClass = jb.getMethod().getDeclaringClass();
                    final ClassModel calledClass = methodRef.getDeclaringClass();
