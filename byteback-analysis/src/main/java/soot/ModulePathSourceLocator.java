...
import soot.asm.model.AsmModuleClassProvider;
...
public class ModulePathSourceLocator extends SourceLocator {
    ...
    public ClassSource getClassSource(String className, Optional<String> moduleName) {
        {
            ...
            if (classesToLoad == null) {
                ...
                for (ClassModel c : ModuleScene.v().getApplicationClasses()) {
    ...
    public Map<String, List<String>> getClassUnderModulePath(String aPath) {
        ...
        if (attrs.isDirectory()) {
            if (!Files.exists(path.resolve(ModuleInfoModel.MODULE_INFO_FILE))) {
    ...
    private Map<String, List<String>> discoverModulesIn(Path path) {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(path)) {
            for (Path entry : stream) {
                ...
                if (attrs.isDirectory()) {
                    if (Files.exists(entry.resolve(ModuleInfoModel.MODULE_INFO_FILE))) {
    ...
    private Map<String, List<String>> buildModuleForJar(Path jar) {
        ...
        try (FileSystem zipFileSystem = FileSystems.newFileSystem(jar, this.getClass().getClassLoader())) {
            Path mi = zipFileSystem.getPath(ModuleInfoModel.MODULE_INFO_FILE);
            if (Files.exists(mi)) {
                InputSource inputSource = new InputSource(mi);

                for (ClassProvider cp : classProviders) {
                    if (cp instanceof AsmModuleClassProvider) {
                        String moduleName = ((AsmModuleClassProvider) cp).getModuleName(inputSource);
                        ModuleInfoModel moduleInfo
                                = (ModuleInfoModel) SootModuleResolver.v().makeClassRef(ModuleInfoModel.MODULE_INFO, Optional.of(moduleName));
                ...
                if (!ModuleScene.v().containsClass(ModuleInfoModel.MODULE_INFO, Optional.of(moduleName))) {
                    moduleInfo = new ModuleInfoModel(ModuleInfoModel.MODULE_INFO, moduleName, true);
                    Scene.v().addClass(moduleInfo);
                    moduleInfo.setApplicationClass();
                } else {
                    moduleInfo = (ModuleInfoModel) ModuleScene.v().getSootClass(ModuleInfoModel.MODULE_INFO, Optional.of(moduleName));
                    if (moduleInfo.resolvingLevel() != ClassModel.DANGLING) {
                ...
                moduleInfo.setResolvingLevel(ClassModel.BODIES);
    ...
    private Map<String, List<String>> buildModuleForExplodedModule(Path dir) {
        ...
        Path mi = dir.resolve(ModuleInfoModel.MODULE_INFO_FILE);

        for (ClassProvider cp : classProviders) {
            if (cp instanceof AsmModuleClassProvider) {
                String moduleName = ((AsmModuleClassProvider) cp).getModuleName(new InputSource(mi));
                ModuleInfoModel moduleInfo
                        = (ModuleInfoModel) SootModuleResolver.v().makeClassRef(ModuleInfoModel.MODULE_INFO, Optional.of(moduleName));
    ...
    public Optional<InputSource> lookUpInModulePath(final String fileName) {
    ...
    protected Optional<InputSource> lookupInDir(final String dirName, final String fileName) {
        Path dirPath = Paths.get(dirName);
        Path foundFile = dirPath.resolve(fileName);
        if (Files.isRegularFile(foundFile)) {
            return Optional.of(new InputSource(foundFile));
        } else {
            return Optional.empty();
        }
    }
    ...
    protected Optional<InputSource> lookupInArchive(final String archivePath, final String fileName) {
        ...
        try (FileSystem zipFileSystem = FileSystems.newFileSystem(archive, this.getClass().getClassLoader())) {
            ...
            if (!Files.isRegularFile(entry)) {
                ...
                return Optional.of(new InputSource(archive.toAbsolutePath().toString(), fileName));
            }
        } catch (IOException e) {
     ...
     * @return the FoundFile
     */
    public Optional<InputSource> lookUpInVirtualFileSystem(String archivePath, String fileName) {
