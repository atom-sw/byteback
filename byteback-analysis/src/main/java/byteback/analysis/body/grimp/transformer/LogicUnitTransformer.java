			...
			public void caseInvokeStmt(final InvokeStmt invokeUnit) {
				...
				if (BBLibNamespace.isContractClass(declaringClass)) {
					...
					final LogicStmt newUnit = switch (method.getName()) {
						case BBLibNamespace.ASSERTION_NAME ->
								Vimp.v().newAssertionStmt(argument);
						case BBLibNamespace.ASSUMPTION_NAME ->
								Vimp.v().newAssumptionStmt(argument);
						case BBLibNamespace.INVARIANT_NAME ->
