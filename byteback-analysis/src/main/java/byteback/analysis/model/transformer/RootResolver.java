...
import byteback.analysis.model.SootClasses;
...
public class RootResolver {
	...
	public void transformMethod(final SootMethod method) {
		...
		if (SootMethods.hasBody(method)) {
			...
			SootBodies.resolveCalls(method.retrieveActiveBody());
			...
			if (!BBLibNamespace.isPureMethod(method) && !BBLibNamespace.isPredicateMethod(method)) {
				...
				if (checkArrayDereference || SootHosts.hasAnnotation(method, BBLibNamespace.MODEL_IOBE_ANNOTATION)) {
					IndexCheckTransformer.v().transform(body);
				}

				if (checkNullDereference || SootHosts.hasAnnotation(method, BBLibNamespace.MODEL_NPE_ANNOTATION)) {
	...
	public boolean classIsValid(final SootClass clazz) {
		return !BBLibNamespace.isAnnotationClass(clazz);
	}
		...
		SootHosts.getAnnotations(method).forEach((tag) -> {
			SootAnnotations.getAnnotations(tag).forEach((sub) -> {
				sub.getElems().forEach((elem) -> {
					new SootAnnotationElems.ClassElemExtractor().visit(elem)
							.ifPresent((classDescriptor) -> {
						final String className = BBLibNamespace.stripDescriptor(classDescriptor);
