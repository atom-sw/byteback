...
Let's see how to use `byteback-tool` to verify the Java program
...
``` bash
javac Main.java -cp ../byteback-specification/build/libs/byteback-annotations.jar
```
...
`byteback-tool` is the command line tool that runs ByteBack. It inputs
Java bytecode and outputs Boogie programs. `byteback-tool` supports the
following options:
...
- `-p PRELUDE` specifies the Boogie prelude files to be used. If this
  ...
  prelude](./byteback-tool/src/main/resources/boogie/BytebackPrelude.bpl).
...
- `--strict` enforces the absence of the listed implicit exceptions.
