 ...
 * @author paganma
 */
public class CallExpr extends AbstractInvokeExpr implements DefaultCaseValue {

	public CallExpr(final SootMethodRef methodRef, final List<Value> args) {
		super(methodRef, new ValueBox[args.size()]);

		for (final ListIterator<? extends Value> valueIterator = args.listIterator(); valueIterator.hasNext();) {
	...
	public boolean equivTo(final Object object) {
		if (object instanceof CallExpr callExpr) {
			if ((this.argBoxes == null ? 0 : this.argBoxes.length) != (callExpr.argBoxes == null ? 0
					: callExpr.argBoxes.length)
