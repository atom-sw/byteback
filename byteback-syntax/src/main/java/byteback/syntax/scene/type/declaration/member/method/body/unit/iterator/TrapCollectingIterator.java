...
/**
 * Wraps a unit iterator to provide information on which traps are active at the
 * current unit.
 *
 * @author paganma
 */
public class TrapCollectingIterator implements Iterator<Unit> {
	...
	public TrapCollectingIterator(final Chain<Unit> units, final Iterable<Trap> traps) {
		...
		for (final Trap trap : traps) {
			final ArrayDeque<Trap> startedTraps = startToTraps.computeIfAbsent(trap.getBeginUnit(),
					($) -> new ArrayDeque<>());
			startedTraps.add(trap);
			final ArrayDeque<Trap> endedTraps = endToTraps.computeIfAbsent(trap.getEndUnit(), ($) -> new ArrayDeque<>());
			endedTraps.add(trap);
		}
	}
