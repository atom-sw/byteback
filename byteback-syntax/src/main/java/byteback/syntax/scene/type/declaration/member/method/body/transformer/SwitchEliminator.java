...
public class SwitchEliminator extends BodyTransformer {
	...
	private PatchingChain<Unit> expandSwitchStmt(final Value base, final Unit defaultTarget,
		...
		while (valueToTargetIterator.hasNext()) {
			...
			final EqExpr condition = Jimple.v().newEqExpr(base, value);
			final IfStmt ifStmt = Jimple.v().newIfStmt(condition, target);
	...
	public void transformBody(final Body body) {
		...
		while (unitIterator.hasNext()) {
			...
			if (unit instanceof final SwitchStmt switchStmt) {
				...
				final Chain<Unit> expandedSwitch = expandSwitchStmt(switchStmt.getKey(), switchStmt.getDefaultTarget(),
						valueToTarget.iterator());
